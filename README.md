...
> **一个以真实对话打磨 AI 角色扮演体验的长期实验项目** — 纯前端 + Capacitor Android 打包

三位性格各异的 AI 女友是三个长期测试角色。借由与她们的真实对话，持续打磨 System Prompt 体系与对话机制，让互动更接近真人相处的质感——角色立得住、记忆对得上、节奏像真人；日常对话中暴露的问题（人设漂移、记忆错乱、语气失真等）逐条记录、复盘、迭代，沉淀成可复用的规则。

## 设计思路

- **真实感优先**：分层人格（Layer 0-5）、结构化共同记忆、消息节奏控制、记忆自动沉淀，每一处都在为"像真人"服务
- **问题驱动迭代**：日常对话中发现的问题记入 Correction 层，反复出现的升级为 Layer 规则，形成「发现 → 记录 → 修复 → 回归」的闭环
...
日常对话中留意这些典型问题：人设漂移（说了不像她的话）、记忆错误（张冠李戴）、语气失真（突然变 AI 腔/总结腔）、节奏崩坏（一口气说完所有话）、越界内容。处理流程：
